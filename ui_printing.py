...
def ui_output(timecode_batch=None, date=None, ee_loc=None, csv_indxs=None):
    img_base = '/home/kantor-lab/Documents/i2grow_central_computer'
    ...
    try:
        if timecode_batch != None and date != None:
            ...
            for i in range(len(timecode_batch)):
                if timecode_batch[i] != None:
                    pathloc = '/home/kantor-lab/Documents/i2grow_central_computer/rgbd_frames/color-image-'+ test_date + '-' + timecode_batch[i] + '.npy'
                    picture_npy = np.load(pathloc)
    ...
    try:
        if date != None:
            ...
            data = pd.read_csv('/home/kantor-lab/Documents/i2grow_central_computer/sensor-readings.csv')
            #print(csv_indxs)
            if csv_indxs == None:
                idxs = []
                timestamps = []
...
if __name__ == "__main__":
    ...
    full_image = cv2.resize(full_image, (1850,1020),interpolation=cv2.INTER_CUBIC)
    while True:
